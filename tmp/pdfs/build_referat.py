...
def make_visuals() -> tuple[dict, dict[str, object]]:
    # Values were generated by the project's deterministic Python engine on
    # 2026-09-14 from bot/data/sample_tenders.json, lot goszakup-700417-ОК1.
    result_data = {
        "Автоматты таңдау": (62.7, 14_546_227, 17.3, 277_920, 13_191_387),
        "Автофура": (60.6, 14_184_302, 16.9, 721_575, 13_635_042),
        "Теміржол": (62.0, 14_215_156, 16.9, 277_920, 13_588_987),
        "Авиа": (41.5, 5_105_056, 6.1, 11_900_000, 24_764_933),
        "30% аванс": (73.4, 14_813_931, 17.6, 277_920, 0),
    }
    results = {
        name: SimpleNamespace(
            ...
            net_profit=profit,
            margin_pct=margin,
            costs=SimpleNamespace(logistics=logistics),
            cash_flow_gap=deficit > 0,
            max_deficit=deficit,
        )
        for name, (tos, profit, margin, logistics, deficit) in result_data.items()
    }
    ...
    baseline_points = [(0, 51.16), (5, 51.71), (10, 14.26), (15, 14.02), (18, -10.92), (20, -11.02), (25, -11.26), (30, -11.78), (35, -12.03), (40, -12.27), (45, -12.51), (50, -12.75), (55, -13.00), (60, 70.76), (65, 66.55)]
    advance_points = [(0, 51.16), (5, 76.91), (10, 39.46), (15, 39.22), (20, 14.18), (25, 13.94), (30, 13.42), (35, 13.17), (40, 12.93), (45, 12.69), (50, 12.45), (55, 12.20), (60, 70.76), (65, 66.81)]
    chart = Image.new("RGB", (1800, 850), "white")
    d = ImageDraw.Draw(chart)
    left, top, right, bottom = 170, 80, 1730, 700
    values = [value for _, value in baseline_points + advance_points]
    ...
    for series, color in ((baseline_points, "#C0392B"), (advance_points, "#1E8449")):
        pts = [xy(day, value, 66) for day, value in series]
...
def add_title_page(doc: Document) -> None:
    ...
    for _ in range(4):
        doc.add_paragraph()
    rows = [
        ("Орындаған", "Байбосынов Нұрали Айбарович"),
        ("Ғылыми жетекші", "Нургалиева Гульжан Абеновна"),
    ]
...
def add_market(doc: Document) -> None:
    add_heading(doc, "1 МЕМЛЕКЕТТІК САТЫП АЛУДАҒЫ ШОБ МӘСЕЛЕСІ")
...
def add_methodology(doc: Document) -> None:
    add_heading(doc, "2 ЗЕРТТЕУ ӘДІСТЕМЕСІ")
...
def add_architecture(doc: Document) -> None:
    add_heading(doc, "3 QAZAQ TENDERS ЖҮЙЕСІНІҢ АРХИТЕКТУРАСЫ")
...
def add_model(doc: Document) -> None:
    add_heading(doc, "4 ЭКОНОМИКАЛЫҚ МАТЕМАТИКАЛЫҚ МОДЕЛЬ")
    ...
    add_equation(doc, [_sub("C", "total"), _mr(" = "), _sub("C", "purchase"), _mr(" + "), _sub("C", "logistics"), _mr(" + "), _sub("C", "tax")], "")
    add_equation(doc, [_mr("Π = S − ("), _sub("C", "total"), _mr(" + "), _sub("C", "bank"), _mr(" + "), _sub("C", "operation"), _mr(" + "), _sub("C", "penalty"), _mr(")")], "1 формула  Болжамды таза пайда")
    add_equation(doc, [_sub("M", "rel"), _mr(" = "), _frac([_mr("Π")], [_mr("S")]), _mr(" · 100%")], "")
    add_equation(doc, [_sub("M", "score"), _mr(" = min(100, max(0, "), _frac([_sub("M", "rel")], [_mr("20%")]), _mr(" · 100))")], "2 формула  Маржа және нормаланған маржа балы")
...
def add_logistics(doc: Document) -> None:
    add_heading(doc, "5 ЛОГИСТИКА ЖӘНЕ ЖЕТКІЗУШІЛЕР МОДУЛІ")
    ...
    add_body(doc, "Бұл тарифтер тасымалдаушының коммерциялық офертасы емес. Автомобиль тасымалы үшін ATI.SU API қолжетімді болғанда live нарықтық ставка қолданылады және интерфейсте дереккөз бөлек белгіленеді. API қолжетімсіз болса, жүйе есептік тарифке қауіпсіз ауысады. Теміржол мен авиа тарифтері әзірге тек модельдік бағдар болып қалады.")
...
def add_case(doc: Document, results: dict, context: dict[str, object]) -> None:
    add_heading(doc, "6 ЕСЕПТІК КЕЙС ЖӘНЕ СЦЕНАРИЙЛІК ТАЛДАУ")
...
def add_validation(doc: Document) -> None:
    add_heading(doc, "7 БАҒДАРЛАМАЛЫҚ ВАЛИДАЦИЯ")
...
def add_limitations(doc: Document) -> None:
    add_heading(doc, "8 ШЕКТЕУЛЕР ЖӘНЕ ДАМЫТУ БАҒЫТТАРЫ")
...
def build() -> None:
    ...
    props.author = "Байбосынов Нұрали Айбарович"
