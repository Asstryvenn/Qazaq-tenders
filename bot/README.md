...
Кнопка «Поставщики» использует только актуальные данные подключённого партнёрского каталога.
Укажите `SUPPLIER_CATALOG_URL` и `SUPPLIER_CATALOG_API_KEY`; контракт ответа документирован в
`suppliers.py`. При отсутствии интеграции бот не показывает вымышленные предложения.
...
Пользователь ──► Telegram ──► bot.py (aiogram 3, FSM, inline-кнопки)
             ...
             голос / PDF / DOCX / ссылка
                  ...
                  Whisper · поиск → SearchQuery · документ → TenderSpec (строгая JSON-схема)
                                  │  факты + помеченные Smart Defaults; без финансового решения
                                  ▼
