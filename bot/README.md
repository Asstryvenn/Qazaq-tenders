...
Кнопка «Поставщики» использует актуальные данные подключённого партнёрского каталога.
Укажите `SUPPLIER_CATALOG_URL` и `SUPPLIER_CATALOG_API_KEY`; контракт ответа документирован в
`suppliers.py`. При отсутствии интеграции, ответе 503 или пустой выдаче бот показывает пять
явно помеченных `ДЕМО · Smart AI` карточек. Они нужны только для проверки сценария, никогда не
считаются верифицированными и не повышают Confidence Level.
...
Пользователь ──► Telegram ──► bot.py (aiogram 3, FSM, inline-кнопки)
             ...
             голос / PDF / DOCX / ссылка
                  ...
                  Whisper · поиск → SearchQuery · документ → TenderSpec (строгая JSON-схема)
                                  ...
                                  ▼
                  calculator.py + logistics.py — Economic Layer (NumPy / Pandas, без LLM)
...
Логистические ставки (`450 ₸/км + 50% обратка`, `180 ₸/км`, `12 000 ₸/т за 1000 км + терминал`,
`850 ₸/кг`) — настраиваемые рыночные ориентиры, а не коммерческие предложения перевозчиков.
...
├── logistics.py         # фура, Газель, Ж/Д, авиа и доставка внутри города
...
- `/digest` — топ-3 лота для вашей сферы; под каждой карточкой есть полный разбор, ссылка,
  симулятор, поставщики, логистика и скрытие лота.
- `🚚 Логистика` → выбор фуры, Газели, Ж/Д или авиа одним нажатием; карточка тут же
  пересчитывает стоимость, срок, пеню, прибыль, TOS и Cash Flow.
